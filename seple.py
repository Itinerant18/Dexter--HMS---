            ...
            try:
                for zone_data in data:    
                    ...
                    if zoneId:
                        ...
                        # Only process selectedDevice if it's not None
                        if selectedDevice:
            ...
            try:    
                for zone_data in data:
                    ...
                    if zoneId:
                        ...
                        file_path = "powerZoneSettings.txt"
...
def general_data():
    if request.method == 'POST':
        ...
        if action == 'change_password':
            ...
            if not all([current_password, new_password, confirm_password]):
                ...
                if db:
                    try:
                        ...
                        error_message = f'Error updating password: {str(e)}'
                    finally:
                        cursor.close()
                        db.close()
                else:
            ...
            if not new_date:
                ...
                try:
                    # Here you would implement the actual system date change
                    # This might require system-level permissions
                    success_message = 'Date updated successfully'
                except Exception as e:
                    error_message = f'Error updating date: {str(e)}'
        
        else:
            # Handle brand and site name update
    ...
    # Redirect back to the general page with messages
...
def control():
    if request.method == 'POST':
        try:
            ...
            # Here you would implement the actual hardware control
        ...
        if request.method == 'POST':
            try:
               # Here we need to add the logic to provision the device
                ...
                success_message = "Device provisioning initiated successfully"
            except Exception as e:
                error_message = f"Error during provisioning: {str(e)}"
...
def save_device_credentials():
    ...
    try:
        ...
        if 'deviceKey' in request.form:
            ...
            if not device_key:
                error_message = 'Device Key is required'
            else:
                # Connect to database
                db = create_connection(db_file)
                if not db:
                    ...
                    try:
                        ...
                        success_message = "e-SIM device credentials saved successfully"
            ...
            # Ethernet form was submitted
            device_type = 'ethernet'
            device_key = request.form.get('ethernetDeviceKey')
            device_secret = request.form.get('ethernetDeviceSecret')
                ...
                if not db:
                    ...
                    try:
                        ...
                        if existing:
                            cursor.execute('''
                                UPDATE device_credentials 
                                SET device_key=?, device_secret=?, access_token=?, device_name=? 
                                WHERE type=? AND id=1
                            ''', (device_key, device_secret, access_token, device_name, device_type))
                        else:
                            cursor.execute('''
                                INSERT INTO device_credentials 
                                (id, type, device_key, device_secret, access_token, device_name) 
                                VALUES (1, ?, ?, ?, ?, ?)
                            ''', (device_type, device_key, device_secret, access_token, device_name))
                        ...
                        if db2:
                            cursor2 = db2.cursor()
                            log_message = "Ethernet device credentials updated"
                            cursor2.execute("INSERT INTO systemLogs (date, message) VALUES (datetime('now', 'localtime'), ?)", (log_message,))
                            db2.commit()
                            cursor2.close()
                            db2.close()
                        
                        success_message = "Ethernet device credentials saved successfully"
                    except Exception as db_error:
                        error_message = f"Database error: {str(db_error)}"
                    finally:
                        cursor.close()
    ...
    # Redirect back to the device credentials page with success/error message
    return redirect(url_for('device_credential', success_message=success_message, error_message=error_message))
...
def integration_settings():
    if 'username' in session:
        ...
        try:
            ...
            print(f"Error retrieving integration data: {e}")
...
def save_integration():
    ...
    try:
        ...
        if not all([device_type, ip_address, device_name, port_number]):
            ...
            if not db:
                ...
                # Determine table name based on device type
                if device_type == 'hikvision-nvr':
                    ...
                    error_message = 'Invalid device type'
                ...
                if not error_message:
                    try:
                        ...
                        # Make sure port_number is an integer
                        try:
                            port_number = int(port_number)
                        except ValueError:
                            port_number = 8000  # Default if not valid
                        
                        # Update or insert record
                        if existing:
                            cursor.execute(f"UPDATE {table_name} SET ip_address=?, device_name=?, device_password=?, port_number=?, is_active=1 WHERE id=1", 
                                        (ip_address, device_name, device_password, port_number))
                        else:
                            cursor.execute(f"INSERT INTO {table_name} (ip_address, device_name, device_password, port_number, is_active) VALUES (?,?,?,?,1)", 
                                        (ip_address, device_name, device_password, port_number))
                        ...
                        if db2:
                            cursor2 = db2.cursor()
                            log_message = f"Integration configuration updated for {device_type}"
                            cursor2.execute("INSERT INTO systemLogs (date, message) VALUES (datetime('now', 'localtime'), ?)", (log_message,))
                            db2.commit()
                            cursor2.close()
                            db2.close()
                        ...
                        # Set success message
                        device_name_display = "HIKVISION NVR" if device_type == "hikvision-nvr" else "HIKVISION BACS" if device_type == "hikvision-bacs" else "DAHUA NVR"
                        success_message = f"{device_name_display} configuration saved successfully!"
                    except Exception as db_error:
                        error_message = f"Database error: {str(db_error)}"
                    finally:
                        cursor.close()
    ...
    # Redirect back to the integration settings page with success/error message
    return redirect(url_for('integration_settings', success_message=success_message, error_message=error_message))
